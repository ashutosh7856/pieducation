...
export default function Home() {
  ...
  const counts = countByStream();

  // Compare tool: a manageable, recognisable subset rather than all 192.
  const comparePool = PRIMARY_STREAMS.flatMap((s) => topByStream(s, 12)).sort((a, b) =>
  ...
  const marqueeNames = PRIMARY_STREAMS.flatMap((s) =>
    topByStream(s, 5).map((c) => c.short_name || c.name),
  );
  ...
  return (
    <>
      ...
      {PRIMARY_STREAMS.map((stream, i) => {
        const list = topByStream(stream, 10);
